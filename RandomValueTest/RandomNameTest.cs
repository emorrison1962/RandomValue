...
namespace RandomValueTest
{
	...
	public class RandomNameTest
	{
		...
		[TestMethod()]
		public void NextFirstMale()
		{
			const int MAX = 1000;
			for (int i = 0; i < MAX; ++i)
			{
				var name = RandomName.NextFirst(RandomName.Gender.Male);
		...
		[TestMethod()]
		public void NextFirstFemale()
		{
			const int MAX = 1000;
			for (int i = 0; i < MAX; ++i)
			{
				var name = RandomName.NextFirst(RandomName.Gender.Female);
				Assert.IsFalse(string.IsNullOrEmpty(name));
				//Debug.WriteLine(name);
			}
		}
